...
point_estination = input(
    "Shopkeeper - Tell me, where are you going for fishing? (river or sea): "
)
...
wants_to_buy = input("Do you want to buy a fishing hook? ").lower()
...
if wants_to_buy == "yes":
    ...
    while True:
        stock_check = input(
            "Customer - Is the hook available \nShoopkeeper: (yes/no)? "
        )
        for stock_check in [stock_check.lower()]:
            if stock_check == "yes":
                for stock_check in range(1):
                    hook_quantity = int(
                        input(
                            "Shoopkeeper - Tell me how many hooks do you want to buy? "
                        )
                    )
                    print(
                        f"Shoopkeeper - Here is your {hook_quantity} {hook_type} hooks. "
                    )
...
rod = input("Do you want to buy a fishing rod? (yes/no): ").lower()

if rod == "yes":
    rod_buy = input(
        "Shopkeeper - Are you sure that you want to buy a fishing rod? (yes/no): "
    ).lower()
    if rod_buy == "yes":
        ...
        if color_rod == "yes":
            ...
            print(
                f"Shopkeeper - Here is your {rod_color} {rod_type} rod. Quantity: {quantity}. "
            )
        else:
            print(
                f"Shopkeeper - Here is your standard {rod_type} rod. Quantity: {quantity}. "
            )
    else:
    ...
    rod_type = input(
        "Which type of rod do you want to buy (metal, bronze, plastic, aluminum): "
    )
    color_rod = input("Do you want a colored rod? (yes/no): ").lower()
    if color_rod == "yes":
        rod_color = input("Which color do you want? ")
...
wants_to_buy_boat = input(
    "Shopkeeper - Do you also want to buy a boat? (yes/no): "
).lower()

if wants_to_buy_boat == "yes":
    boat_size = input("Which size of boat do you want (small/medium/large)? ").lower()
    if boat_size == "small":
...
# Bank account
print(
    "Shopkeeper - You are going out of the city, so you need much cash for it. Do you have enough cash for spending there? \n"
)

is_atm_available = input("Is there any ATM machine there: (yes/no): ").lower()

if is_atm_available == "yes":
    account_available = input("Do you have any bank account? (yes/no): ").lower()
    if account_available == "no":
        open_bank_account = input(
            "Shopkeeper - Do you want to open a bank account? (yes/no): "
        ).lower()
        if open_bank_account == "yes":
            ...
            your_number = input(
                "Is your number registered on your name? (yes/no): "
            ).lower()
            if your_number == "no":
                print("Okay, I converted it to your number. ")
            elif your_number == "yes":
                print("It's perfect. ")
...
# Hook purchase input
hooks_buy = input("Did you buy a hooks? (yes/no):").lower()
if hooks_buy == "yes":
...
if rod_buy == "yes":
    rod_type = input(
        "Which type of rod did you buy (metal, bronze, plastic, aluminum)? "
    ).lower()
    ...
    if rod_type == "metal":
        ...
        print(
            "Invalid rod type entered. Please enter a valid rod type (metal, bronze, plastic, or aluminum)."
        )
    total_bill += rod_total_cost
else:
    print("Okay, lets move on")

boat_buy = input("Did you buy a boat (yes/no): ").lower()
if boat_buy == "yes":
...
print(
    "Shopkeeper - Thank you for your purchase! Have a great day and enjoy your fishing trip!"
)
